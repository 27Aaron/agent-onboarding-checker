...

{
  ...
  "riskTriggers": [
    "命中风险的具体触发点，3-5 条，用「触发点：为什么危险」写"
  ],
  "permissions": [
...
- riskTriggers 不能留空，要解释为什么这个任务触发风险
...
function syncRiskDetailsVisibility() {
  const shouldOpen = riskToggleBtn.getAttribute("aria-expanded") === "true";
  riskDetails.hidden = !shouldOpen;
}

function setRiskDetails(items) {
  const finalItems = toList(items).slice(0, 6);
  renderList(
    riskDetails,
    finalItems.length ? finalItems : ["暂无明确触发点，建议补充任务描述后重新体检。"],
  );
  syncRiskDetailsVisibility();
}

function deriveAiRiskDetails(enhancement, permissionItems, approvalItems) {
  if (enhancement.riskTriggers.length) return enhancement.riskTriggers;
  const reasonItems = enhancement.riskReason ? [`主要风险：${enhancement.riskReason}`] : [];
  const approvalDetails = approvalItems.map((item) => `需要人工确认：${item}`);
  const permissionDetails = permissionItems.map((item) => `需要临时权限：${item}`);
  return [...reasonItems, ...approvalDetails, ...permissionDetails].slice(0, 6);
}
...
function parseAiEnhancement(output) {
  ...
  return {
    ...
    riskTriggers: toList(data.riskTriggers || data.triggers || data.riskPoints || data.riskDetails),
...
function applyAiEnhancement(output) {
  ...
  const hasStructuredContent =
    hasStructuredPolicy ||
    enhancement.riskTriggers.length > 0 ||
  ...
  setRiskDetails(deriveAiRiskDetails(enhancement, finalPermissions, finalApprovals));
...
function analyze() {
  ...
  renderList(permissions, finalPermissions);
  renderList(approvals, finalApprovals);
  setRiskDetails(buildRiskDetails(matched));
  permissionCount.textContent = `${finalPermissions.length} 项`;
  approvalCount.textContent = `${finalApprovals.length} 项`;
  updateSummaryPreviews(finalPermissions, finalApprovals);
